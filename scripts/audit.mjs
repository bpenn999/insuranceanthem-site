...
// pages" is not good enough — it has to be identical and complete everywhere.
{
  ...
  for (const f of htmls) {
    ...
    // The practice inbox — the ONE address the site publishes. The footer NAP
    // is the citation surface, so it has to match what GBP and the directories
    // carry, and that is the role address. No personal brian@ anywhere.
    if (!footer.includes('help@602medicare.com')) note(`FOOTER: no email on ${r}`);
