...
def fingerprint(tmp: Path, stem: str) -> str:
    """Rename the built file to include a content hash, and drop stale ones.

    THIS IS NOT COSMETIC. public/_headers serves /fonts/* with
    `max-age=31536000, immutable`, so a font published under a name that has
    ever been served is frozen for a YEAR — at the edge and in every browser
    that has seen it. This was found the hard way: Fraunces was re-issued 45%
    smaller under the same filename and every visitor kept the old one.

    A content hash makes the name change exactly when the bytes do, which is the
    only version of this that cannot be got wrong by forgetting.
    """
    digest = hashlib.sha1(tmp.read_bytes()).hexdigest()[:8]
    final = tmp.with_name(f"{stem}-{digest}.woff2")
    if final.exists():
        tmp.unlink()
    else:
        tmp.rename(final)
...
def write_manifest(built: dict[str, str]) -> None:
    """Emit the hashed filenames for BaseHead to import.

    Generated rather than hand-written for the same reason as the hash itself:
    a filename that changes on every rebuild cannot be maintained by hand.
    """
    out = ROOT / "src" / "data" / "fonts.ts"
    body = (
        "/**\n"
    ...
    print("\nwrote src/data/fonts.ts:")
    for k, v in sorted(built.items()):
        print(f"  {k:<10} /fonts/{v}")
...
def main() -> None:
    ...
    for stem in ("fraunces", "inter"):
        ...
        subset(src, tmp, chars)
        if stem == "fraunces":
            pin_optical_size(tmp)
        built[stem] = fingerprint(tmp, stem)
